...
- “持仓盈亏”和“交易盈亏”字段名称保持不变。
- 持仓盈亏只计算收盘时仍然持有的昨仓，不包含日内新仓：
  - `持仓盈亏 = 收盘剩余昨仓数量 × (今收 - 昨收) × 多空方向 × 合约乘数`。
  - 昨仓全天未动时，等同于 `昨仓数量 × (今收 - 昨收) × 多空方向 × 合约乘数`。
  - 昨仓日内部分或全部平仓时，已平仓部分不再计入持仓盈亏。
- 交易盈亏只由实际平仓行为产生：
  - `交易盈亏 = 实际平仓数量 × (平仓价格 - 当时加权平均持仓成本) × 多空方向 × 合约乘数`。
  - 开仓成交不会产生交易盈亏。
  - 同方向加仓会更新加权平均持仓成本；部分平仓不改变剩余仓位的持仓成本。
