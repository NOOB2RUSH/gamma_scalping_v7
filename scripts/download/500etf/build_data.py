    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt == retries:
                break
            print(
                f"[500etf] retry {func.__name__} {attempt}/{retries}: "
                f"{type(exc).__name__}",
                flush=True,
            )
            time.sleep(sleep)
...
def fetch_etf_history(start, end):
    ...
    try:
        ...
        print(
            f"[500etf] fund_etf_hist_em failed, fallback to sina: {type(exc).__name__}",
            flush=True,
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "下载南方中证500ETF(510500)与对应 ETF 期权数据。"
...
def parse_contract_id(contract_id):
    match = re.fullmatch(r"510500([CP])(\d{2})(\d{2})([A-Z]?)(\d+)", contract_id)
    if match is None:
        raise ValueError(f"无法解析 500ETF 期权合约交易代码: {contract_id}")
...
def discover_contracts_from_risk(start, end, trading_calendar, request_sleep):
    ...
    for idx, date in enumerate(dates, start=1):
        ...
        try:
            ...
            risk_df = risk_df[risk_df["CONTRACT_ID"].astype(str).str.startswith("510500")]
            ...
            print(
                f"[500etf] risk {idx}/{len(dates)} {date_text}: "
                f"active={len(risk_df)}, unique={len(metadata)}",
                flush=True,
            )
        except Exception as exc:
            errors.append({"date": date_text, "code": "", "error": repr(exc)})
            print(f"[500etf] risk {date_text} failed: {type(exc).__name__}", flush=True)
...
def discover_current_contracts(trading_calendar, request_sleep):
    metadata = {}
    errors = []
    months = ak_call(ak.option_sse_list_sina, symbol="500ETF")
    for month in months:
            ...
            try:
                ...
                for code in codes[code_col].astype(str):
                    try:
                        ...
                        contract_id = (
                            f"510500{option_type}{month[2:]}M"
...
def download_options(start, end, metadata, output_dir, request_sleep, skip_existing):
    ...
    for idx, (code, meta) in enumerate(items, start=1):
        ...
        if idx % 25 == 0 or idx == len(items):
            print(f"[500etf] contracts {idx}/{len(items)}", flush=True)
...
def main():
    ...
    print(
        f"[500etf] etf days={etf_df['date'].nunique() if not etf_df.empty else 0}, "
    ...
    print(f"[500etf] output: {output_root}", flush=True)
