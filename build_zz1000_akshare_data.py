...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "下载并转换中证1000股指期权 AKShare 数据。"
            "期权历史日线没有 bid/ask，因此统一使用 close 近似 bid=ask=mid。"
        )
    )
    parser.add_argument("--start", required=True, help="开始日期，例如 20260519")
    parser.add_argument("--end", required=True, help="结束日期，例如 20260521")
    parser.add_argument(
        "--months",
        nargs="*",
        default=None,
        help="要下载的期权月份，例如 mo2606 mo2607；默认使用 AKShare 当前返回的全部月份。",
    )
    parser.add_argument(
        "--auto-months",
        action="store_true",
    ...
    parser.add_argument(
        "--month-start",
        default=None,
        help="自动探测月份起点，例如 202207。",
    )
    parser.add_argument(
        "--month-end",
        default=None,
        help="自动探测月份终点，例如 202601。",
    )
    parser.add_argument(
        "--request-sleep",
        type=float,
...
def load_trading_calendar():
    index_df = ak.stock_zh_index_daily(symbol=UNDERLYING_SYMBOL)
    return pd.DatetimeIndex(pd.to_datetime(index_df["date"])).sort_values()


def get_available_months():
    months = ak.option_cffex_zz1000_list_sina()
...
def discover_historical_months(month_start, month_end, request_sleep):
    ...
    for month in iter_month_codes(month_start, month_end):
        try:
            snapshot = ak.option_cffex_zz1000_spot_sina(symbol=month)
...
def download_underlying(start, end, output_dir):
    ...
    index_df = index_df[(index_df["date"] >= start_ts) & (index_df["date"] <= end_ts)]
    ...
    for date, row in index_df.groupby("date"):
        ...
        file_path = (
            output_dir
            / f"{UNDERLYING_FILE_PREFIX}_{date.strftime('%Y-%m-%d')}_price.parquet"
        )
...
def build_contract_history(code, start, end, trading_calendar):
    daily = ak.option_cffex_zz1000_daily_sina(symbol=code)
    if daily.empty:
        return pd.DataFrame()
    ...
    # AKShare 单合约历史没有历史 bid/ask，这里按约定使用 close 近似 mid。
    return pd.DataFrame(
        {
            "date": daily["date"],
            "order_book_id": code,
...
def download_options(
    ...
    if frames:
        option_df = pd.concat(frames, ignore_index=True)
    else:
...
def main():
    ...
    index_df = download_underlying(start, end, index_dir)
    option_df, errors = download_options(
        start,
        end,
        months,
        option_dir,
